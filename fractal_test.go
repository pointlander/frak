...
func TestFractal(t *testing.T) {
	...
	decoded := FractalDecoder(fcpBuffer, 4)
...

	{
		...
		unsplit := image.NewRGBA(input.Bounds())
		for x := 0; x < width; x++ {
			for y := 0; y < height; y++ {
				r, _, _, _ := r.At(x, y).RGBA()
				g, _, _, _ := g.At(x, y).RGBA()
				b, _, _, _ := b.At(x, y).RGBA()
				unsplit.Set(x, y, color.RGBA{
					R: uint8(r >> 8),
					G: uint8(g >> 8),
					B: uint8(b >> 8),
					A: 0xFF})
		...
		file, err = os.Create(name + "_unsplit.png")
		if err != nil {
			log.Fatal(err)
		}

		err = png.Encode(file, unsplit)
		if err != nil {
			log.Fatal(err)
		}
		file.Close()
